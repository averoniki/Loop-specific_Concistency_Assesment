		...
		for(i in 1:no.loops) {
			...
			if(is.null(network.tau.sq) & common.tau==T){
				...
				if(nrow(data.metareg)==3){
					...
					tau.squared<-o$tau2
					if(is.null(tau.squared)){
					  o<- try(rma(data.metareg$theta1, data.metareg$var.theta, mods=cbind(comp.a,comp.b,comp.c),data=data.metareg,intercept=FALSE,method=method, control=list(stepadj=0.5)), silent=TRUE)
					  tau.squared<-o$tau2
					}else{tau.squared}
				}
				for(j in 1:loop.dimension) {
